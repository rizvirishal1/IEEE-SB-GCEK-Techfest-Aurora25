const NavBarLarge = ({ handleNavigation, getPath }) => {
  ...
  const menu = ["Home", "About", "Events", "Login"];
  const paths = ["/", "/about", "/events", "/login"];

  return (
    <div className={container}>
      <h1 className={header}>AURORA'25</h1>
...
function MenuItem({ name, path, getPath, handleNavigation }) {
  ...
  return (
    <span
      ...
      className={`rounded-[35px] cursor-pointer text-white px-5 py-[10px] text-2xl font-semibold ${
        path === getPath() || isOnUserDashboard
          ? "backdrop-blur-md bg-green-500/30 shadow-lg shadow-green-500/20 border border-white/40"
          : ""
      }`}
    >
      {name}
